...
from intent_comm_nav_vendor.controllers.navigation_noisy_rational_human import NavigationNoisyRationalHuman
...
class HighLevelRunnerNode(Node):
    def __init__(self):
        super().__init__("high_level_runner")
        ...
        self.topic_r_u_obs = str(self.declare_parameter("topic_r_u_obs", "/hl/robot/u_obs").value)
        self.topic_h_cmd   = str(self.declare_parameter("topic_h_cmd", "/human/cmd_vel").value)
        self.topic_r_cmd   = str(self.declare_parameter("topic_r_cmd", "/robot/cmd_vel").value)
        ...
        # NEW: controller selector (only thing you pass via CLI besides trial_id in launch)
        ...
        self.beta_h = float(self.declare_parameter("beta_h", 1.0).value)
        ...
        sigma2_h = list(self.declare_parameter(
        ...
        self.sigma2_state_h = tuple(float(x) for x in sigma2_h)
        ...
        # Blame-Me params (kept separate but editable in launch; defaults match your sim demo)
        ...
        # ----------------------
        # Human controller (same)
        # ----------------------
        self.human = NavigationNoisyRationalHuman(
            theta_self=self.theta_h_true,
            ctrl_index_self=0,
            intents=tuple(self.intents_r),
            dt=self.dt_hl, horizon=self.horizon,
            goals_self=self.goals_h,
            goals_opp=self.goals_r,
            ...
            beta=self.beta_h, stochastic=True, seed=42,
            beta_state=self.beta_state, rho_forget=self.rho_forget,
            sigma2_state=self.sigma2_state_h,
        )

        # ----------------------
        # Robot controller (NEW switch)
        # ----------------------
        self.robot_kind = "npace"
        if self.controller in ("blame_me", "blameme", "blame-me"):
            if NavigationBlameMeController is None:
                self.get_logger().error("controller=blame_me requested but NavigationBlameMeController import failed. Falling back to NPACE.")
            else:
        ...
        self.sub_r_u_obs = self.create_subscription(Twist, self.topic_r_u_obs, self._on_r_u_obs, 20)

        self.pub_h_cmd = self.create_publisher(Twist, self.topic_h_cmd, 10)
        ...
        self._r_u_obs: Optional[np.ndarray] = None

        self._ready = False
        self._last_u_h = np.zeros(2, dtype=np.float32)
        self._last_u_r = np.zeros(2, dtype=np.float32)
        self._tick_i = 0

        self.timer = self.create_timer(self.dt_hl, self._tick)

        self.get_logger().info("Warmup: compiling controllers in background...")
        threading.Thread(target=self._warmup_worker, daemon=True).start()

        self.get_logger().info(
            f"HighLevelRunner controller={self.robot_kind} dt_hl={self.dt_hl:.3f}s horizon={self.horizon} swap_agents={self.swap_agents} | "
            f"x_hat={self.topic_x_hat} | h_u_obs={self.topic_h_u_obs} | r_u_obs={self.topic_r_u_obs} | "
            f"cmd_h={self.topic_h_cmd} cmd_r={self.topic_r_cmd}"
        )

    def _warmup_worker(self) -> None:
        x0 = np.array([-4.0, 0.0, 0.0,  4.0, 0.0, np.pi], dtype=np.float32)
        u0 = np.array([0.0, 0.0], dtype=np.float32)
        try:
            uh = self._call_human(obs=x0, a2=u0)
            ur = self._call_robot(obs=x0, a1=u0)
            try:
                import jax
                ...
                pass
            self._ready = True
            self.get_logger().info("Warmup: controller compute_action compiled OK.")
    ...
    def _on_r_u_obs(self, msg: Twist) -> None:
        self._r_u_obs = _twist_to_u(msg)

    def _call_human(self, obs: np.ndarray, a2: np.ndarray) -> np.ndarray:
        fn = self.human.compute_action
        try:
            return fn(obs=obs, a_opponent_observed=a2)
        except TypeError:
            try:
                return fn(obs=obs, a2_observed=a2)
            except TypeError:
                return fn(obs, a2)
    ...
    def _tick(self) -> None:
        ...
        r_u_obs = self._r_u_obs if self._r_u_obs is not None else np.zeros(2, dtype=np.float32)

        x = self._x_hat.copy()

        if not self._ready:
            u_h = np.zeros(2, dtype=np.float32)
            u_r = np.zeros(2, dtype=np.float32)
        else:
            try:
                _t0 = time.perf_counter()
                u_h = self._call_human(obs=x, a2=r_u_obs.copy())
                ...
                u_h, u_r = self._last_u_h, self._last_u_r

        u_h = np.array([np.clip(u_h[0], self.v_lo, self.v_hi), np.clip(u_h[1], self.w_lo, self.w_hi)], dtype=np.float32)
        u_r = np.array([np.clip(u_r[0], self.v_lo, self.v_hi), np.clip(u_r[1], self.w_lo, self.w_hi)], dtype=np.float32)

        self._last_u_h, self._last_u_r = u_h, u_r

        self.pub_h_cmd.publish(_u_to_twist(u_h))
        ...
        try:
            b_h = self.human.belief_over_theta
            msg_bh = Float32MultiArray()
            msg_bh.data = [float(b_h.get(k, 0.0)) for k in self.intents_r]
            self.pub_b_human.publish(msg_bh)
        except Exception:
            pass

        try:
            # NPACE: robot_belief_over_human
            # Blame-Me: belief_over_theta
            b_r = None
        ...
        if self._tick_i % max(1, int(round(1.0 / self.dt_hl))) == 0:
            self.get_logger().info(
                ...
                f"u_h=[{u_h[0]:+.2f},{u_h[1]:+.2f}] u_r=[{u_r[0]:+.2f},{u_r[1]:+.2f}]"
            )


def main(args=None):
    rclpy.init(args=args)
    node = HighLevelRunnerNode()
    try:
        rclpy.spin(node)
